import { Box, Button, FormControl, Heading, Image, Stack, Text, Textarea, useToast } from '@chakra-ui/react'
...
const Tweet = ({ tweets, getTweets }) => {
    ...
    const onSubmit = async () => {
        try {
            if (!userId.username) return toast({
                title: 'Please login.',
            ...
            <Stack spacing={4} p='2rem' boxShadow='dark-lg' borderRadius={20} border='-5px' alignItems='center'>
                ...
                <FormControl width='500px' padding='20px'>
                    <Box display='flex' alignItems='center' >
                        <IoPerson size='25px' />
                        <Text m='10px' fontWeight='20px'>{userId.username}</Text>
                    </Box>
                    <Textarea placeholder='tweet ur post' height='200px' maxLength='150' value={tweet} onChange={handleInputChange}> </Textarea>
                    <Box display='flex' my='20px'>
                        <Text>{tweet.length}/150</Text >
                        <Box mx='5px' p='5px'><FaRegImages /></Box>
                        <Box mx='5px' p='5px'> <MdInsertEmoticon /></Box>
                        <Box mx='5px' p='5px'> <IoDocumentTextOutline /></Box>
                        <Box mx='5px' p='5px'> <MdOutlineKeyboardVoice /> </Box>
                    </Box>
                </FormControl>
